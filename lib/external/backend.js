  ...
  const _requestBrowser = rasterizerPort => new Promise((accept, reject) => {
    const _recurse = i => {
      if (i < BROWSER_BIN_PATHS.length) {
        ...
        const browserVersionProcess = child_process.execFile(browserBinPath, [
          '--version',
        ], {
          encoding: 'utf8',
        }, (err, s) => {
          if (!err) {
            const match = s.match(/^(Google Chrome|Chromium) ([0-9]+)/);

            if (match) {
              const browserVersion = parseInt(match[2], 10);

              if ((isWindows && browserVersion >= 60) || (!isWindows && browserVersion >= 59)) {
