...
const CHROMIUM_BIN_PATH = isWindows ?
    path.join(__dirname, '..', '..', 'bin', 'windows', 'chrome.exe')
  :
    path.join(__dirname, '..', '..', 'bin', 'linux', 'usr', 'bin', 'chromium');
...
const rasterize = ({
  ...
  const _requestRasterizer = rasterizerPort => new Promise((accept, reject) => {
    const chromiumProcess = child_process.spawn(CHROMIUM_BIN_PATH, [
      '--headless',
