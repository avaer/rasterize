...
const render = (src, width, height) => {
  ...
  const _requestMetadata = () => {
    ...
    const measures = (() => {
      const result = {};
      const measures = Array.from(divEl.querySelectorAll('[measure]'));
      for (let i = 0; i < measures.length; i++) {
        const measure = measures[i];
        const id = measure.getAttribute('measure');
        const {innerText} = measure;
        const {font} = getComputedStyle(measure);
        const entry = Array(innerText.length + 1);
        for (let j = 0; j <= innerText.length; j++) {
          entry[j] = _measureText(innerText.slice(0, j), font);
        }
        result[id] = entry;
      }
      return result;
